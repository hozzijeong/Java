...
public class Inventory {
	...
	public void inventoryMenu() {
		while (true) {
			...
			if (sel == 1) {
				if(itemList.size() ==0) {
					System.out.println("[메세지] 착용가능한 아이템이 없습니다.");
					break;
				}
				equipMenu();
			}
			if(sel == 2) {
				if(itemList.size() ==0) {
					System.out.println("[메세지] 판매가능한 아이템이 없습니다.");
					break;
				}
				sellMenu();
			}
	...
	public void equipMenu() {
		...
		while (true) {
			...
			if (itemList.get(selEquip).kind == Item.WEAPON) {
				...
				Player.getGuildUnit(selUnit-1).att += itemList.get(selEquip).power;
				...
				Player.getGuildUnit(selUnit-1).def += itemList.get(selEquip).power;
				...
				Player.getGuildUnit(selUnit-1).hp += itemList.get(selEquip).power;
				Player.getGuildUnit(selUnit-1).maxHp += itemList.get(selEquip).power;
